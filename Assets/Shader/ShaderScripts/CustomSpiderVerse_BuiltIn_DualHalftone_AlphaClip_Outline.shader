Shader "Custom/SpiderVerse_BuiltIn_DualHalftone_AlphaClip_Outline"
{
    Properties
    {
        ...
        _ShadowBands("Shadow Bands", Range(1,6)) = 3
        ...
        _RimPower("Rim Power", Range(0.5,12)) = 1.6
        _RimStrength("Rim Strength", Range(0,2)) = 1.2

        // Halftone Layer A (Dots)
        _HalftoneScale("Halftone Scale (Dots)", Range(10,400)) = 180
        _HalftoneStrength("Halftone Strength (Dots)", Range(0,2)) = 0.35
        _HalftoneThreshold("Halftone Threshold (Dots)", Range(0,1)) = 0.60
        _HalftoneSoftness("Halftone Softness (Dots)", Range(0.001,0.5)) = 0.08
        ...
        _HatchScale("Hatch Scale", Range(20,600)) = 240
        _HatchStrength("Hatch Strength", Range(0,2)) = 0.28
        _HatchThreshold("Hatch Threshold", Range(0,1)) = 0.68
        _HatchSoftness("Hatch Softness", Range(0.001,0.5)) = 0.12
        _HatchLineWidth("Hatch Line Width", Range(0.01,0.5)) = 0.12
        _HatchAngle("Hatch Angle (Deg)", Range(0,180)) = 35
        [Toggle]_CrossHatch("Cross Hatch (2nd Angle)", Float) = 1

        // Halftone distance fade (prevents distracting crawl)
        ...
        // AO (only meaningful if AO is packed into RGB)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vertOL(appdata v)
            {
                ...
                clipPos.z -= _OutlineDepthOffset * clipPos.w; // reduce z-fighting
            ...
            fixed4 fragOL(v2f i) : SV_Target
            {
                ...
                // Edge-only outline: strong near cutoff, weak in solid interior
        ...
        // ---------------- FORWARD BASE (Toon + Shadows + Dual Halftone) ----------------
        Pass
        {
            Name "ForwardBase"
            ...
            // returns 1 where ink line is, 0 where gap is
            half HatchLines(float2 uv, half scale, half lineWidth, half angleDeg)
            {
                ...
                // 0 at line center, 1 at cell edges
                float wave = abs(frac(v) - 0.5) * 2.0;

                // ink line around center
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // AO (optional / only meaningful if packed)
                ...
                // Dual halftone driving factor (shadows)
                half shadowiness = 1.0h - ramp;

                // Distance fade (stops distracting crawl)
