...
// Attached to the Psychic Slam projectile prefab. Handles self-propulsion, trigger detection,
...
public class Mb_PsychicSlamProjectile : MonoBehaviour
{
    ...
    [SerializeField] private float _TravelSpeed = 75f;

    [Tooltip("How far the projectile travels before deactivating.")]
    [SerializeField] private float _MaxRange = 75f;
        ...
        bool isOvercharged)
    {
        ...
        // --- Scale visual root ---
        // ShockwaveVisual is the visible projectile mesh/VFX anchor.
        _visualRoot = transform.Find("ShockwaveVisual");
        if (_visualRoot != null)
            _visualRoot.localScale = new Vector3(width, height, depth);
